...
{
	return container_of_const(panel, struct tianma_622_v0, panel);
}
...
static int tianma_622_v0_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(tianma_622_v0_supplies), ctx->supplies);
	if (ret < 0) {
...
static int tianma_622_v0_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct tianma_622_v0, panel,
				   &tianma_622_v0_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(tianma_622_v0_supplies),
					    tianma_622_v0_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
